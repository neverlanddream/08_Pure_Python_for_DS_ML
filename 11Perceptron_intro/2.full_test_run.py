...
def output_weights(train, learning_rate, how_many_epoch):
    ...
    for epoch in range(how_many_epoch):
        ...
        for row in train:
            ...
            error = prediction - row[-1]  # 输入的数据结构里最后一列/维度就是结果值（0 or 1）
            sum_error += error ** 2
    ...
    return weights # weights列表是w0 w1 w2 ...,其中w0也是bias
...
weights = output_weights(dataset, learning_rate, how_many_epoch)
